...
def background_subtract(aligned, out_dir):
    ...
    for f in aligned:
        ...
        bkg = sep.Background(data)      # ⚠️ default settings, not tuned
        data_sub = data - bkg           # ⚠️ subtract full background, not bkg.back()
...
def detect_and_match_gaia_RAW(wcs_solved_files, snr_threshold=1.1):
    ...
    for f in wcs_solved_files:
        ...
        print(f"  📊 Background RMS (RAW): {bkg_rms:.2f} counts")
        print(f"  🎯 Detection threshold: {snr_threshold}σ = {snr_threshold * bkg_rms:.2f} counts")
        ...
        print(f"  🔍 SEP detections (RAW): {n_detections}")
        ...
        # Convert pixel → world coordinates (using RAW WCS)
        ...
        print(f"  ⭐ {n_gaia} matched to Gaia")
        print(f"  🎯 {n_candidates} asteroid candidates")
        
        if n_candidates > 0:
            candidate_snrs = df[~df["matched_to_gaia"]]["snr"]
            print(f"  📈 Candidate SNR range: {candidate_snrs.min():.1f} - {candidate_snrs.max():.1f}")
...
def convert_detections_to_aligned_frame(detections, wcs_solved_files, aligned_files):
    ...
    for det_df, wcs_file, aligned_file in zip(detections, wcs_solved_files, aligned_files):
        ...
        print(f"  🔄 Converting {len(det_df)} detections to aligned coordinates...")
        ...
        # Convert: RAW pixels → RA/Dec → ALIGNED pixels
        ...
        print(f"  ✅ Converted to aligned frame")
...
def save_tracks_to_master(tracks, folder, aligned, times, pix_scale, master_csv):
    ...
    if tracks is None or tracks.empty:
        print(f"⚠️ No tracks found for {folder.name}, skipping append.")
        return
    ...
    print(f"💾 Added {len(tracks)} tracks → {master_csv}")
...
def run_pipeline_single(folder, master_csv):
    ...
    if len(wcs_solved) < 3:
        print(f"⚠️ {folder.name}: missing WCS-solved FITS (found {len(wcs_solved)})")
        return
    if len(aligned) < 3:
        print(f"⚠️ {folder.name}: missing aligned FITS (found {len(aligned)})")
        return

    try:
        times = extract_frame_times(wcs_solved)      # returns MJD values
        dt1 = (times[1] - times[0]) * 86400      # seconds between frame1–frame2
        dt2 = (times[2] - times[1]) * 86400      # seconds between frame2–frame3
        print(f"🕒 Cadence detected: Δt1={dt1/60:.2f} min, Δt2={dt2/60:.2f} min")
    except Exception as e:
        print(f"⚠️ Cadence extraction failed ({e}), using default 120 s")
        dt1, dt2 = 120.0, 120.0

    print("\n🔍 Step 1: Detecting on RAW WCS-solved images...")
    detections_raw, pix_scale = detect_and_match_gaia_RAW(
        wcs_solved, 
        snr_threshold=1.1  # Mookodi's default - no need to lower!
    )

    print("\n🔄 Step 2: Converting detections to aligned coordinates...")
    ...
    if output_rows:
        ...
        print(f"✅ Saved SEP + Gaia detections to: {out_path}")
    else:
        print(f"⚠️ No detections to save for {folder.name}")

    # ============================================================
    # 3B. REMOVE GAIA STARS, SHOW PLOT, AND PREPARE DETECTIONS FOR LINKER
    # ============================================================

    print("\n🔭 Processing Gaia-filtered detections (no reprojection)...")
    ...
    for i, (df, bg_file) in enumerate(zip(detections, aligned)):
        if df is None or df.empty:
            print(f"⚠️ Frame {i+1}: No detections found.")
        ...
        print(f"📸 Frame {i+1}: {len(df)} detections → "
        ...
        try:
            ...
            print(f"⚠️ Plotting failed for frame {i+1}: {e}")

        # Add to detections list for linker
        updated_detections.append(non_gaia_df)

    detections = updated_detections
    print(f"✅ Gaia-filtered detections ready for motion linking "
        f"({sum(len(df) for df in detections if df is not None)} total candidates).")

    # ============================================================
    # 🔬 DIAGNOSTICS — Gaia alignment only (truth-free)
    #   Computes σ_align_px from Gaia star–star nearest-neighbor residuals.
    # ============================================================
    ...
    def _gaia_alignment_residuals(dfA, dfB, max_pair_px=8.0):
        """
        Star–star alignment residuals using Gaia-matched detections (truth-free).
    ...
    try:
        ...
        print("📐 STAR–STAR ALIGNMENT (Gaia NN residuals)")
        print(f"  F1→F2: n={r12['n']}, median={r12['median_px']:.2f}px, p90={r12['p90_px']:.2f}px")
        print(f"  F2→F3: n={r23['n']}, median={r23['median_px']:.2f}px, p90={r23['p90_px']:.2f}px")
        ...
        print(f"📐 STAR–STAR ALIGNMENT skipped ({_e})")
        sigma_align_guess = 1.5

    print(f"📏 Using sigma_align_px ≈ {sigma_align_guess:.2f} px for prediction radius.")
    ...
    if not (len(C1) and len(C2) and len(C3)):
        print("⚠️ One or more frames have zero candidates; skipping linker.")
        ...
        # Times → seconds relative to t1 (constant-velocity uses dt ratios; seconds is robust)
        ...
        for tr in linked:
            rows.append({
                ...
                "disp12_px": tr["disp12_px"],    # ✅ ADD THIS
                "disp23_px": tr["disp23_px"],    # ✅ ADD THIS
            })
        tracks = pd.DataFrame(rows)

    print(f"✅ Motion linker (v2) produced {len(tracks)} three-frame track(s).")
    ...
    if not expected_cols.issubset(tracks.columns):
        print("❌ Track format error - missing coordinate columns")
        return
    ...
    print(f"🧮 Tracks summary: {len(tracks)} total")
...
def safe_run_pipeline_single(subfolder, master_csv, error_log="skipped_folders.txt"):
    ...
    try:
        ...
        print(f"⚠️ Skipped {subfolder.name} due to error: {e}")
...
def run_pipeline_all(root="cleaned_data",
    ...
    if include_csv is not None:
        ...
        # map of {folder_name -> Path} for fast lookup and to keep root’s sort order
        ...
        # Log any names in CSV that don’t exist under root
        if missing:
            with open(error_log, "a") as f:
                for m in missing:
                    f.write(f"{m}: listed in CSV but not found under root\n")
            print(f"⚠️ {len(missing)} folder(s) listed in CSV not found under root. Logged in {error_log}.")
    else:
        folders = all_folders
    ...
    print(f"📂 Processing {len(folders)} folders (index {start} to {end-1})")
    for i, sub in enumerate(folders, start=start):
        print(f"\n[{i}] --- {sub.name} ---")
        safe_run_pipeline_single(sub, master_csv, error_log=error_log)

    print(f"\n✅ Master CSV saved at {master_csv}")
    print(f"📝 Skipped/invalid folders logged in {error_log}")